...
- **Card density in the client file**: a minimal card (name, date, total) with options/important-info always visible as badges without a click. In the global "Réservations" view (all clients, by date), full detail always shows regardless of status. (An earlier version of this rule described a distinct "Brouillon" full-detail card — that distinction was dropped and should not be reintroduced; `statut_resa` can still technically be "Brouillon", but it gets no special card treatment.)
