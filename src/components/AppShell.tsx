...
function AppShellInner({
  ...
  const { topVenteIds, topRentabiliteIds } = useMemo(() => {
    ...
    reservationsVendues(allReservations).forEach((r) => {
      ...
      const impacts = allRemboursements.filter((rb) => rb.activite_id === r.id).map(remboursementImpact);
      ...
      byItem[r.catalogue_item_id].total += total + impactCa;
      byItem[r.catalogue_item_id].marge += total - cout + impactMarge;
    });
  ...
  }, [allReservations, clients, allResaOptions, allResaTarifs, allCoutsMap, allRemboursements, isDirection]);
  ...
  return (
    <div className="flex min-h-screen bg-[#fafafa]">
      ...
      {mode === "team" && (
        <div className="flex flex-1 flex-col">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#666666]/10 bg-white px-3 py-1.5">
            <div className="flex w-full min-w-0 items-center gap-2 sm:w-auto sm:flex-1">
              <button
                ...
                className="hidden h-[30px] w-7 flex-shrink-0 items-center justify-center rounded-md text-neutral-500 hover:bg-[#fafafa] md:flex"
              >
              ...
              <input
                ...
                className="min-w-0 flex-1 rounded-md border border-neutral-300 px-2 py-1.5 text-sm focus:border-[#171717] focus:outline-none sm:w-56 sm:flex-none"
              />
              <QuickAddClient
                onCreate={addClient}
                onUpdateClient={updateClientById}
        ...
        {(
          [
            ...
            },
            {
              ...
              label: "Pick up",
              icon: IconChecklist,
              active: mode === "suivis" && suivisSub === "j1",
              onClick: openPickupsChambres,
            },
          ] as const
        ).map((t) => {
          const Icon = t.icon;
          return (
            <button
              ...
              className={`flex flex-1 flex-col items-center gap-0.5 py-2 text-[10px] font-medium ${
                t.active ? "text-[#8B4531]" : "text-[#666666]"
              }`}
            >
              <Icon />
              {t.label}
