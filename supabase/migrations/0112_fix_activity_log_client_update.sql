-- URGENT — corrige une régression introduite par 0111 : le CASE unique
-- (when 'clients' then new.id else new.client_id end) fait planter TOUTE
-- modification de la table clients ("record new has no field client_id"),
-- exactement le bug déjà corrigé une première fois par 0010 il y a
-- longtemps — 0111 l'a réintroduit sans le savoir en réécrivant toute la
-- fonction. On repasse par des branches IF/ELSIF distinctes (jamais new/
-- old.client_id évalué quand TG_TABLE_NAME = 'clients'), en gardant cette
-- fois la construction de la description ajoutée par 0111.
...
-- Même raison qu'en 0035/0036 : un trigger combiné AFTER sur delete plante
-- dès qu'un client (ou une ligne parente) est supprimé en cascade — la
-- ligne n'existe déjà plus quand le trigger tente d'insérer dans
-- activity_log. On sépare insert/update (AFTER) de delete (BEFORE), comme
