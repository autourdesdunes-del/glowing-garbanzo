-- Type de paiement déverrouillé pour tout le monde (plus seulement la
-- Direction) — pour ne jamais perdre le choix initial même s'il change en
-- cours de route, on le fige dans paiement_type_prevu (jamais réécrit une
-- fois posé) et on trace le changement dans l'historique du dossier.
alter table clients add column if not exists paiement_type_prevu text not null default '';
